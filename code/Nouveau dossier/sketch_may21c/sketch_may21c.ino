...
void loop() {
  ...
  // Lecture du pH
  float pH_value = measurePH();

  // Déterminer le statut acide/basique/neutre
  String pH_status = determinePHStatus(pH_value);
...
void displayOnLCD1(float t, float h, float pH_value, String pH_status) {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Temp:");
  lcd.print(t);
...
String determinePHStatus(float pH_value) {
  if (pH_value < 7.0) {
    return "Acide";
